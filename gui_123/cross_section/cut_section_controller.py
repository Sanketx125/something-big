...
class CutSectionController:
    ...
    def _build_cut_unified_actor(self, points, classes, point_size=3):
        ...
        try:
            ...
            try:
                rw = getattr(self.cut_vtk, "render_window", None)
                if rw is not None and _try_enable_program_point_size(rw):
            ...
            try:
                ...
                _a, _c, _w = actor, ctx, self.cut_vtk
                QTimer.singleShot(
                    500,
                    lambda: _deferred_actor_gpu_init(_a, _c, _w, "CutSection")
                )
            except Exception:
                pass
    ...
    def _refresh_cut_colors_fast(self):
        ...
        try:
            from vtkmodules.util import numpy_support
            from gui.unified_actor_manager import _mark_actor_dirty, sync_palette_to_gpu
                ...
                if polydata:
                    ...
                    if vtk_colors and vtk_colors.GetNumberOfTuples() == len(classes):
                        ...
                        if class_vtk_arr is not None:
                            ...
                            cached_cls = getattr(target_actor, "_naksha_section_class", None)
                            if (
                                cached_cls is not None
                                and cached_cls is not cls_ptr
                                and getattr(cached_cls, "shape", None) == cls_ptr.shape
                            ):
                                np.copyto(cached_cls, cls_ptr)
