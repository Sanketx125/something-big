...
#                 # Without this, camera sync stays permanently disabled after
#                 # the first cross-section, causing stale camera states.
#                 # ═══════════════════════════════════════════════════════════
#                 self.app._syncing_camera = prev_syncing

#                 # ═══════════════════════════════════════════════════════════
#                 # ✅ FIX: Force 2D camera as FINAL step after all rendering
#                 # This prevents any intermediate 3D/tilted state from
#                 # leaking through sync observers or actor rebuilds.
#                 # ═══════════════════════════════════════════════════════════
#                 try:
#                     vtk_widget = self._get_active_vtk()
#                     if vtk_widget:
...
#         Cross-section views must ALWAYS use parallel projection with the camera
#         looking straight at the section plane. This method enforces that constraint
#         without changing the zoom level or pan position — it only fixes the
#         projection mode and view orientation if they've drifted.
...
#             # ── 1. Always force parallel projection ──────────────────────
#             if not cam.GetParallelProjection():
#                 cam.ParallelProjectionOn()
#                 print(f"   🔧 Forced ParallelProjection ON for View {(view_index or 0) + 1}")

#             # ── 2. Fix view-up if it has drifted from (0, 0, 1) ──────────
#             up = cam.GetViewUp()
#             # Allow small numerical drift (< 0.01) but fix real tilts
...
#             # ── 3. Ensure camera looks along the correct axis ────────────
#             view_mode = getattr(self.app, 'cross_view_mode', 'side')
#             pos = cam.GetPosition()
#             fp = cam.GetFocalPoint()

#             if view_mode == 'side':
#                 # Camera should look along -Y: position.y < focal_point.y
#                 # Fix if the Y-axis relationship is wrong (camera not behind the plane)
#                 if abs(pos[1] - fp[1]) < 1.0:
#                     # Camera too close to focal plane on Y — push it back
...
#             else:
#                 # front view: camera looks along -X
...
#             # ✅ FIX: Final 2D enforcement after all camera operations
#             self._force_section_camera_2d(vtk_widget, view_index)
