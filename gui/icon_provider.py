...
def _load_icon_from_file(icon_path: Path, size: int) -> QIcon:
    ...
    if icon_path.suffix.lower() == ".svg":
        try:
            ...
            if icon_path.name in _LARGE_CANVAS_EXTERNAL_SVGS:
                scale = 1.18
            return _render_svg_bytes(icon_path.read_bytes(), size, scale=scale)
        except Exception:
...
_SCOPED_EXTERNAL_RIBBON_ICON_FILES = {
    ("MeasurementRibbon", "Actions", "Clear"): "icon-clear.svg",
}


_EXTERNAL_RIBBON_ICON_FILES = {
    ("File", "Open"): "icon-open.svg",
...
def get_external_ribbon_icon_path(button_text: str, section_title: str = None,
    ...
    filename = _SCOPED_EXTERNAL_RIBBON_ICON_FILES.get((ribbon_scope, section_title, button_text))
    if not filename:
        filename = _EXTERNAL_RIBBON_ICON_FILES.get((section_title, button_text))
    if not filename:
        return None
...
def get_button_icon(button_text: str, section_title: str = None,
    ...
    if external_icon_path is not None:
        return _load_icon_from_file(external_icon_path, size)
