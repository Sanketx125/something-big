...
# snt_attachment.py
# =================
# SNT Binary CAD Overlay dialog for NakshaApp  v2.1
# Sanket NakshaTech — NakshaApp native format attachment UI.

# Mirrors dxf_attachment.py pattern exactly:
#   - SNTFileItem             ↔  DXFFileItem
#   - MultiSNTAttachmentDialog ↔  MultiDXFAttachmentDialog
#   - SNTLayerSelectionDialog  ↔  DXFLayerSelectionDialog
#   - SNTDisplayOptionsDialog  ↔  DXFDisplayOptionsDialog
#   - show_snt_attachment_dialog(app) ↔  show_multi_dxf_attachment_dialog(app)

# v2.1 fixes vs v2.0
# ───────────────────
#   FIX-G1  _convert_entities: replaced hardcoded 'DW'/'MURAGLIONE' text
#            filter with the same smart priority-based label detection used
#            in DXF process_entity → INSERT → candidate_labels logic.
#            SNT text labels now get identical colour/height treatment as DXF.

#   FIX-G2  _render_snt_in_vtk: after registering actors to app.snt_actors,
#            also registers an identical entry to app.dxf_actors so that the
#            existing grid-click → LAZ-load pipeline (which reads dxf_actors)
#            can find SNT actors without any changes to downstream code.

#   FIX-G3  _attach_all: dxf_attachments bridge already present in v2.0;
#            now also ensures dxf_actors is properly populated per-file at
#            render time (via FIX-G2) rather than trying to bulk-add after
#            the fact (which missed per-file full_path resolution).

#   FIX-G4  _remove_from_vtk: already correct in v2.0 — removes from both
#            snt_actors and dxf_actors. Preserved unchanged.

#   FIX-F1..F10 from v2.0 all preserved.
# """
...
#                     indeterminate,
#                 )
#                 item_data: Dict = {"snt_path": snt_path}
#                 try:
...
#         if item in self.snt_items:
#             self.snt_items.remove(item)
#             self.file_list_layout.removeWidget(item)
#             self._remove_from_vtk(item.snt_path.name)
#             item.deleteLater()
#             self._update_file_count()

#     def _remove_from_vtk(self, filename: str):
#         """
#         Remove VTK actors and attachment records from BOTH snt_actors/snt_attachments
#         AND dxf_actors/dxf_attachments so no stale references remain.
#         """
#         try:
#             target = os.path.basename(filename)

#             # Try to get renderer safely
#             renderer = None
#             try:
...
#             # Remove from snt_actors
#             for store_name in ['snt_actors', 'dxf_actors']:
#                 store = getattr(self.app, store_name, [])
#                 for i in range(len(store) - 1, -1, -1):
#                     if os.path.basename(store[i].get("filename", "")) == target:
#                         if renderer:
#                             for actor in store[i].get("actors", []):
#                                 try:
...
#                         a for a in getattr(self.app, list_name)
...
#         except Exception as exc:
#             print(f"  ⚠️ Remove SNT from VTK failed: {exc}")

#     def _clear_all(self):
#         for item in list(self.snt_items):
...
#     # ── FIX-G2: VTK rendering with dual actor registration ────────────────

#     # def _render_snt_in_vtk(self, attachment: Dict) -> None:
#     #     """
#     #     Render all SNT entities into VTK.

#     #     KEY CHANGE vs v2.0:
#     #       After building the actor list, we register it in BOTH app.snt_actors
#     #       AND app.dxf_actors using identical dict schemas.  This means every
#     #       downstream consumer that reads dxf_actors (grid-click handler, Grid
#     #       Label Manager, classification refresh, optimized_refresh) will see
#     #       SNT files transparently — no changes required in those systems.
#     #     """
#     #     try:
#     #         import vtk

#     #         try:
#     #             from gui.optimized_refresh import (
#     #                 tag_actor_as_dxf, invalidate_dxf_actor_cache)
#     #         except ImportError:
#     #             def tag_actor_as_dxf(a):         pass
#     #             def invalidate_dxf_actor_cache(): pass

#     #         if not hasattr(self.app, "vtk_widget") or not self.app.vtk_widget:
#     #             print("  ⚠️ VTK widget not ready")
#     #             return

#     #         renderer  = self.app.vtk_widget.renderer
#     #         actors:   List = []
#     #         item      = attachment.get("_snt_item")
#     #         is_under  = attachment["mode"] == "underlay"
#     #         entities  = attachment.get("entities", [])

#     #         if not entities:
#     #             print(f"  ℹ️ No entities to render for {attachment['filename']}")
#     #             _entry = {"filename":  attachment["filename"],
#     #                       "full_path": attachment.get("full_path"),
#     #                       "actors":    []}
#     #             self.app.snt_actors.append(_entry)
#     #             self.app.dxf_actors.append(_entry)   # ← bridge (empty)
#     #             return

#     #         # ── Polygon offset helper ─────────────────────────────────────
#     #         def _offset(mapper, v=-100.0):
#     #             try:
#     #                 mapper.SetResolveCoincidentTopologyToPolygonOffset()
#     #                 mapper.SetRelativeCoincidentTopologyPolygonOffsetParameters(v, v)
#     #             except Exception:
#     #                 pass

#     #         # ── Actor cache helper ────────────────────────────────────────
#     #         def _cache(actor, layer_name):
#     #             if item is not None:
#     #                 item.actor_cache.setdefault(layer_name, []).append(actor)
#     #                 item.entity_layers[len(item.entity_layers)] = layer_name

#     #         # ── Partition ─────────────────────────────────────────────────
#     #         face_ents  = [e for e in entities if e["type"] == "3dface"]
#     #         line_ents  = [e for e in entities if e["type"] in ("line", "polyline")]
#     #         text_ents  = [e for e in entities if e["type"] == "text"]
#     #         point_ents = [e for e in entities if e["type"] == "point"]

#     #         print(f"\n📊 SNT Rendering: {attachment['filename']}\n"
#     #               f"  3DFACE={len(face_ents)}  Lines={len(line_ents)}  "
#     #               f"Text={len(text_ents)}  Points={len(point_ents)}")
...
#     #                     for i in range(3): cell.GetPointIds().SetId(i, i)
#     #                 else:
#     #                     cell = vtk.vtkQuad()
...
#     #                 edges.SetInputData(pd); edges.Update()
#     #                 appender.AddInputData(edges.GetOutput())
#     #             appender.Update()
#     #             mapper = vtk.vtkPolyDataMapper()
#     #             mapper.SetInputData(appender.GetOutput())
#     #             _offset(mapper, -100.0)
#     #             actor = vtk.vtkActor()
#     #             actor.SetMapper(mapper)
#     #             actor.GetProperty().SetColor(_normalise_vtk_color(color))
#     #             actor.GetProperty().SetLineWidth(2.0)
#     #             actor.GetProperty().SetAmbient(0.5)
#     #             actor.GetProperty().SetDiffuse(0.8)
#     #             actor.GetProperty().SetOpacity(0.6 if is_under else 1.0)
...
#     #                     for i in range(n): pline.GetPointIds().SetId(i, i)
#     #                     cells = vtk.vtkCellArray()
...
#     #             actor.GetProperty().SetOpacity(0.5 if is_under else 1.0)
#     #             actor._original_color = color
#     #             tag_actor_as_dxf(actor)
#     #             renderer.AddActor(actor)
#     #             actors.append(actor)
...
#     #                 a.GetProperty().SetOpacity(0.5 if is_under else 1.0)
#     #                 renderer.AddActor(a)
#     #                 actors.append(a)
#     #                 _cache(a, e.get("layer", "0"))
#     #             except Exception as exc:
#     #                 print(f"  ⚠️ Text actor failed: {exc}")

#     #         # ══ STEP 4: Points ══════════════════════════════════════════════
#     #         for e in point_ents:
#     #             try:
#     #                 a = self._create_point_actor(e)
#     #                 if a is None:
#     #                     continue
#     #                 tag_actor_as_dxf(a)
#     #                 a.GetProperty().SetOpacity(0.5 if is_under else 1.0)
...
#     #         # ══ STEP 5: Dual actor registration (FIX-G2) ══════════════════
#     #         #
#     #         # Build ONE shared entry dict and store a reference (not a copy)
#     #         # in both app.snt_actors and app.dxf_actors.  Using the same object
#     #         # means toggling visibility via either list always affects the same
#     #         # actors — no sync issues.
#     #         #
#     #         actor_entry = {
...
#     #         self.app.dxf_actors.append(actor_entry)   # ← FIX-G2: bridge

#     #         # Invalidate optimized_refresh actor cache so it rebuilds with
#     #         # the new SNT actors included on the next classification pass.
#     #         try:
...
#     #             print(f"  ✅ Rendered {len(actors)} actors "
#     #                   f"(faces={len(face_groups)}, "
#     #                   f"lines={len(line_groups)}, "
#     #                   f"text={len(text_ents)}, "
#     #                   f"points={len(point_ents)})")
#     #             print(f"  🔗 Dual-registered in snt_actors + dxf_actors")
#     #         else:
#     #             try:
#     #                 self.app.vtk_widget.render()
#     #             except Exception:
...
#     #     except Exception as exc:
#     #         print(f"  ⚠️ SNT VTK render failed: {exc}")
#     #         traceback.print_exc()
...
#             if not hasattr(self.app, "vtk_widget"): return
#             renderer = self.app.vtk_widget.renderer
#             actors = []
#             entities = attachment.get("entities", [])
...
#                     for i in range(pts.GetNumberOfPoints()): pline.GetPointIds().SetId(i, i)
#                     pd = vtk.vtkPolyData()
#                     pd.SetPoints(pts)
#                     cells = vtk.vtkCellArray()
#                     cells.InsertNextCell(pline)
...
#                 # --- THE CRITICAL FIX ---
#                 # 1. Force the feature ON (Your log showed this was OFF)
#                 mapper.SetResolveCoincidentTopologyToPolygonOffset()
#                 # 2. Use a massive offset for UTM scale
...
#                 # 3. TRANSVERSE PASS HACK: setting to 0.99 forces it to render last
...
#             actor_entry = {"filename": attachment["filename"], "full_path": fpath, "actors": actors}
#             self.app.snt_actors.append(actor_entry)
...
class SNTLayerSelectionDialog(QDialog):
    ...
    def _init_ui(self):
        ...
        title = QLabel("Select Layers to Display")
        title.setStyleSheet(f"font-weight:bold; font-size:11pt; color:{ThemeColors.get('accent')}; padding:5px;")
        layout.addWidget(title)

        self.layer_list = QListWidget()
        self.layer_list.setStyleSheet(f"""
            QListWidget {{ background:{ThemeColors.get('bg_input')}; border:1px solid {ThemeColors.get('border_light')}; border-radius:4px; }}
            QListWidget::item {{ padding:5px; border-bottom:1px solid {ThemeColors.get('border')}; }}
            QListWidget::item:hover    {{ background:{ThemeColors.get('bg_button')}; }}
            QListWidget::item:selected {{ background:{ThemeColors.get('accent')}; }}
        """)
        layout.addWidget(self.layer_list)
...
class MultiSNTAttachmentDialog(QDialog):
    ...
    def _init_ui(self):
        ...
        info = QLabel(
            ...
            "Colours & layers preserved  |  Grid pipeline fully compatible")
        info.setStyleSheet(
            f"color:{ThemeColors.get('text_secondary')}; font-size:9pt; padding:6px;"
            f"background:{ThemeColors.get('bg_secondary')}; border-radius:4px;")
        info.setAlignment(Qt.AlignCenter)
        layout.addWidget(info)
