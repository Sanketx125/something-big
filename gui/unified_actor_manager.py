...
#             "//VTK::Color::Impl", True,
#             "//VTK::Color::Impl\n"
#             "// === Naksha MicroStation Border v13 ===\n"
#             "vec2 uv_v13 = gl_PointCoord.xy - vec2(0.5);\n"
#             "\n"
#             "if (border_ring_val > 0.001) {\n"
#             "    // ── SQUARE sprite + border on ALL points (MicroStation) ───\n"
#             "    //\n"
#             "    // ring_fraction = clamp(border_ring_val × 0.5, 0.0, 0.25)\n"
#             "    // This keeps the border at most 25% of sprite area:\n"
#             "    //   • At 1-2px (zoom out): sub-pixel border → no dark cloud\n"
#             "    //   • At 3-4px (normal):   ~1px border → clean visible frame\n"
#             "    //   • At 6px+ (zoom in):   1-2px border → prominent frame\n"
#             "    float ring_v13 = clamp(border_ring_val * 0.5, 0.0, 0.25);\n"
#             "\n"
#             "    // Chebyshev distance → square shape (matches MicroStation)\n"
#             "    float sq_v13 = max(abs(uv_v13.x), abs(uv_v13.y)) * 2.0;\n"
#             "    if (sq_v13 > 1.0) discard;\n"
#             "    if (sq_v13 >= (1.0 - ring_v13)) {\n"
#             "        diffuseColor = vec3(0.0, 0.0, 0.0);\n"
#             "        ambientColor = vec3(0.0, 0.0, 0.0);\n"
#             "    }\n"
#             "} else {\n"
#             "    // ── No border: plain round circle ─────────────────────────\n"
#             "    if (length(uv_v13) * 2.0 > 1.0) discard;\n"
#             "}\n"
#             "opacity = 1.0;\n"
#             "// === End Naksha Border v13 ===\n",
#             False
#         )
...
#     if actor is None:
#         print(f"   ℹ️ Section {view_idx+1}: no actor yet — building")
#         return build_section_unified_actor(app, view_idx, palette, border_percent) is not None
 
#     rgb_ptr = getattr(actor, '_naksha_rgb_ptr', None)
#     if rgb_ptr is None or not _is_writable(rgb_ptr):
#         print(f"   ⚠️ Section {view_idx+1}: RGB buffer not writable — rebuilding")
#         return build_section_unified_actor(app, view_idx, palette, border_percent) is not None
...
#         return build_section_unified_actor(app, view_idx, palette) is not None
...
#         # Guard: skip actors whose RGB buffer is gone / read-only
#         rgb_ptr = getattr(sec_actor, "_naksha_rgb_ptr", None)
#         if rgb_ptr is None or not _is_writable(rgb_ptr):
#             continue
 
#         # ── FIX: correct attribute written by build_section_unified_actor() ──
#         # Old code: app.section_{view_idx}_combined_mask  → NEVER set, always None
#         # Correct:  app._section_{view_idx}_global_indices → int array set at build
#         gi_section = getattr(app, f'_section_{view_idx}_global_indices', None)
#         if gi_section is None:
#             print(f"   ⚠️  fast_undo_update: _section_{view_idx}_global_indices "
#                   f"not found — skipping section {view_idx + 1}")
#             continue
 
#         # Guard: changed_mask must cover the max global index in this section
#         max_gi = int(gi_section.max(initial=0))
#         if len(changed_mask) < max_gi + 1:
#             # Mask shorter than section references — treat ALL section points
#             # as changed.  Safe fallback, rare in practice.
...
#         _patch_actor_memory(app, sec_actor, local_sec_changed,
#                             slot_idx=view_idx + 1)
 
#         try:
#             vtk_widget.render()
#         except Exception as e:
#             print(f"   ⚠️  fast_undo_update: render failed for section "
#                   f"{view_idx + 1}: {e}")
...
#     overrides = {}
#     dlg = getattr(app, "display_mode_dialog", None)
#     if dlg and hasattr(dlg, "view_palettes") and slot_idx in dlg.view_palettes:
#         overrides = dlg.view_palettes[slot_idx]
#     else:
#         vp = getattr(app, "view_palettes", {})
#         overrides = vp.get(slot_idx, {})
