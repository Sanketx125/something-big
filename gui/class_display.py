...
#     """
#     Convert border_percent (0..100) to ring thickness.
#     More aggressive scaling for highly visible borders.
...
#     """
#     Apply a round point + black ring border using fragment shader.
    
#     Note: Shader code is compiled during render, not immediately.
#     Returns True if the replacement was successfully registered.
#     """
#     if border_percent <= 0:
#         return True
...
#         if not sp:
#             print("      ⚠️ No shader property on actor!")
#             return False
...
#         # Apply the replacement (this just registers it, doesn't compile yet)
...
#         # Force updates - this marks objects as needing shader recompilation
#         sp.Modified()
...
#         # ✅ Trust that the replacement was registered
#         # VTK will compile the actual shader during the next render
#         return True
            
#     except Exception as e:
#         print(f"      ❌ Shader registration failed: {e}")
#         import traceback
#         traceback.print_exc()
#         return False
...
#     #     """Populate combo box with visible classes."""
...
#     #         lvl = info.get("lvl", "")  # <--- Get Lvl
            
#     #         # ✅ Format: "Code: Description (Lvl)"
...
#     #     print(f"✅ Loaded {len(visible_classes)} classes into control panel")
...
#         # ✅ Re-render to show changes (with updated weights)
#         print(f"\n   🔄 Triggering render with new weights...")
#         try:
#             from gui.class_display import update_class_mode
#             update_class_mode(self.app)
#             print(f"   ✅ Render complete - all views updated with new weight")
#         except Exception as e:
...
#     # def calculate_border_size(base_size, border_percent):
#     #     """
#     #     ✅ FIXED: Calculate border size with more visible effect
        
#     #     Args:
#     #         base_size: Base point size (typically 2-4 pixels)
#     #         border_percent: Border percentage (0-50%)
        
#     #     Returns:
#     #         tuple: (border_size, base_size) with visible difference
#     #     """
#     #     if border_percent == 0:
#     #         return None, base_size
        
#     #     # ✅ NEW FORMULA: More aggressive border scaling
#     #     # 5% = +1.0px, 10% = +2.0px, 25% = +5.0px, 50% = +10.0px
#     #     border_offset = (border_percent / 2.5)  # 0-10 pixel range
        
#     #     # Border size = base + offset
#     #     border_size = base_size + border_offset
        
#     #     print(f"      🔳 Border calc: {border_percent}% → base={base_size:.1f}px, border={border_size:.1f}px (Δ={border_offset:.1f}px)")
        
#     #     return border_size, base_size
...
def apply_border_shader_ring(actor, border_percent: float, border_rgb=(0.0, 0.0, 0.0)) -> bool:
    ...
    try:
        ...
        if ring <= 0.001:
            ...
            # ── MicroStation ADAPTIVE SQUARE border ──────────────────────────
            # Per-class actors built outside _attach_view_shader_context do not
            # have the v_point_size varying wired, so we use gl_PointSize which
            # IS readable in the fragment stage on all desktop OpenGL drivers
            # via the built-in gl_PointCoord.  We cannot read gl_PointSize
            # directly in fragment shaders, but VTK passes it through
            # gl_PointCoord.  The workaround: we need to rely on the VTK
            # custom vertex attribute pipeline.
            #
            # For this simpler per-class path we bake the ring as a static
            # constant (same as base zoom).  The unified GPU path (which covers
            # the vast majority of points) handles full adaptive behaviour via
            # v_point_size.  This path only fires for legacy per-class actors.
            code = (
                "//VTK::Color::Impl\n"
                "// === Naksha MicroStation Adaptive Border (per-class) ===\n"
                "vec2  uv_pc  = gl_PointCoord.xy - vec2(0.5);\n"
                f"float ring_pc = {ring:.6f};\n"
                "// Square sprite with border\n"
                "float sq_pc = max(abs(uv_pc.x), abs(uv_pc.y)) * 2.0;\n"
                "if (sq_pc > 1.0) discard;\n"
                "if (sq_pc >= (1.0 - ring_pc)) {\n"
                f"    diffuseColor = vec3({br:.6f}, {bg:.6f}, {bb:.6f});\n"
                "    ambientColor = diffuseColor;\n"
                "}\n"
                "opacity = 1.0;\n"
                "// === End Naksha per-class border ===\n"
            )
