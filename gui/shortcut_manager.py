...
#         # cell = self.table.item(row, 3)
#         # existing_preset = decode_display_preset(cell.data(Qt.UserRole)) if cell else None
#         # ✅ FIXED: Read preset from app.shortcuts (always fresh/synced)
#         # Fallback to table cell only if not found in app.shortcuts
...
#                 print(f"   ✅ Loaded preset from app.shortcuts[{mod}+{key}] (fresh weights)")
        
#         # Fallback to table cell (for newly added rows not yet in app.shortcuts)
#         if existing_preset is None:
#             cell = self.table.item(row, 3)
#             existing_preset = decode_display_preset(cell.data(Qt.UserRole)) if cell else None
#             if existing_preset:
#                 print(f"   ⚠️  Loaded preset from table cell (may have stale weights)")  ###
        
#         # ✅ CRITICAL: Always create a FRESH picker (don't reuse)
#         # This prevents state issues when reopening
...
#             # DisplayModeDialog uses view_borders dict
#             if hasattr(dlg, 'view_borders') and dlg.current_slot in dlg.view_borders:
#                 border_val = dlg.view_borders[dlg.current_slot]
#                 picker.border_spin.setValue(border_val)
#                 print(f"🔗 Synced border from Display Mode: {border_val}%")
#             elif hasattr(dlg, 'view_borders') and 0 in dlg.view_borders:
#                 border_val = dlg.view_borders[0]
#                 picker.border_spin.setValue(border_val)
#                 print(f"🔗 Synced border from Display Mode (Main View): {border_val}%")
#         elif hasattr(self.app_window, 'display_border_percent'):
#             picker.border_spin.setValue(self.app_window.display_border_percent)
#             print(f"🔗 Synced border from app_window: {self.app_window.display_border_percent}%")
...
#             if views:
#                 # ✅ Normalize keys to int (views can have str or int keys)
...
#                 picker.view_selector.setCurrentIndex(first_view_idx)  # int ✅
...
#         else:
#             # ✅ FRESH PRESET: Just set to Main View with default settings
#             print(f"📋 Creating fresh preset - defaulting to Main View")
            
#             picker.view_selector.blockSignals(True)
#             picker.view_selector.setCurrentIndex(0)  # Default to Main View
#             picker.view_selector.blockSignals(False)
            
#             # Populate classes (weights will be set correctly based on view selector)
#             picker._populate_classes()
    ...
    def _on_weight_changed(self, code, value):
        ...
        if code in self.app_window.class_palette:
            ...
            try:
                ...
                fast_palette_refresh(self.app_window, self.app_window.class_palette, border)
            except Exception as e:
